...
- **空镜铁律在 Seedream(Ark) 上反噬（挂账本 #15，实施背景咬合前先修它）**：
  Ark 无负词通道，负词折进正文后，铁律那墙「人、兽、猫狗、怪物」token 是召唤
  不是禁止（樱见坂实弹：运行时 `_bg_prompt` 出的六张背景张张有人，固定 bg_seed
  把同一个女孩复制进六张）。修法方向：ark 路生物负词不折正文、铁律改
  「无人的空景」一句零 token 措辞、生物压制靠地点描述消毒；剧本级 `bg_style`
  用「背景美术」措辞且零生物 token（樱见坂已验证有效）。
