...
| 20 | 08-18 | 每日配额 v1 只闸 /play — phone/call/confront/golden 等旁路 LLM 端点各有冷却但不进日账; drive 拍不落玩家拍也不计数 (有 5s 间隔兜底, 日上限≈1.7万拍) | 🟡 主路已闸 | runs.py `_charge_daily_quota`; v2 = 旁路端点也过闸 (只查不计) 或统一计费表 |
