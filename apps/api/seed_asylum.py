...
THREAT = {
    "char_id": "ay_tie",
    "unfightable": True,   # 逃生法则：打不过。只能跑、躲、绕。
    "senses": ["sound", "light"],
...
ENDINGS = [
    ...
    {"id": "end_broken", "kind": "bad", "trigger": "sanity", "title": "第41例之后",
     "text": "不知从哪一刻起，那些声音不再吓你了。数石子挺好的。一、二、三。"
     "走廊的灯灭了一格又一格，你坐在原地，把石子摆成整整齐齐的一排。"
     "早晨来查房的人在记录上写：情绪平稳，配合治疗。——他们说的是你。"},
]

# 🧠 理智账本：目睹、遭遇、撬开重真相都要付账；安静的回合缓一点回来；归零=崩断
SANITY = {"enabled": True, "name": "理智", "start": 100, "regen": 1,
          "ending_id": "end_broken"}

# 📜 守则（规则怪谈）：贴在护士站软木板上的《夜间守则》。一至三条引擎执法；
# 四、五条没有执法条款——它们是让人睡不着的那种。
RULES = [
    {"id": "ru_light", "text": "一、熄灯后，不得在走廊与病房使用手电或任何强光。",
     "violate": {"keywords": ["开手电", "打开手电", "手电照", "手电直照", "打光", "开灯"],
                 "channel": ["do"]},
     "consequence": {"pressure": 10, "sanity": -3, "threat_aggro": True,
                     "text": "光柱扫过去的那一瞬，整层楼安静了一拍。然后你听见，"
                             "很远的地方，有什么东西转过身来。"}},
    {"id": "ru_sing", "text": "二、听到唱歌时，不要跟着唱。",
     "violate": {"keywords": ["跟着唱", "一起唱", "哼起", "唱起", "接着唱"],
                 "channel": ["say", "do"]},
     "consequence": {"sanity": -6, "pressure": 6,
                     "text": "你唱到第二句才发现，走廊里原来的歌声停了。"
                             "现在整栋楼里，只剩你一个人的声音。"}},
    {"id": "ru_fridge", "text": "三、太平间第三格冷柜，未经批准不得开启。",
     "when": {"location_id": "loc_morgue"},
     "violate": {"keywords": ["第三格", "打开冷柜", "拉开冷柜", "开冷柜"],
                 "channel": ["do"]},
     "consequence": {"sanity": -8, "pressure": 8,
                     "text": "柜门拉开一条缝。里面没有灯。但你确定——有什么东西，"
                             "在里面看着你。你把它推了回去，指尖凉了很久。"}},
    {"id": "ru_name", "text": "四、夜里听到有人叫你的名字，装作没听见。"},
    {"id": "ru_gone", "text": "五、（这一条被人用指甲划掉了，只能辨认出——"
                              "「……不要相信广播」）"},
]
    ...
    {"id": "a1", "index": 1, "title": "无尽长夜", "time": {"day": 1, "slot": "夜"},
     ...
     "events": [
        {"id": "e_lockdown", "what_happens": "你翻进三楼病房区的当口，身后的防火铁门'咔'地自锁。"
         "走廊灯次第熄灭，只剩尽头一格惨白的应急灯。手机还亮着，信号格是满的——却拨不出这座山。",
         "who_character_ids": []},
...
def main() -> None:
    ...
    try:
        ...
        story = Story(
            ...
            synopsis="自由记者沈默循着线报夜探寂声疗养院，防火门在身后自锁，手机拨不出外线。"
            ...
            tuning={"max_new_characters": 6, "plan_render": 1,
                    # 🎨 画风：所有生图（场景/头像/随手拍）统一走恐怖片美术方向
                    "art_style": "阴郁冷调的恐怖片美术：低饱和青绿与病态的荧光灯白，"
                    "深重阴影吃掉画面边缘，胶片噪点，潮湿剥落的墙皮与发黄旧瓷砖质感，"
                    "构图冷静对称、留出令人不安的空旷，绝不出现血浆与怪物"},
            visibility="public",
        )
        db.add(story)
        db.flush()
