...
def test_anniversary_fires_on_month_mark_and_replaces_invite():
    ...
    nw = next(n for n in st.get("living_news") or [] if n.get("kind") == "anniv")
    # ⚖️ 只攒不演: 那条亲笔讯息是词债, 心跳时手机不响
    assert nw["pend"]["msg_due"] is True
    assert not ((st.get("phone") or {}).get("threads") or {}).get("a")
    living.settle_pending(STORY, st, MockLLM())
    thread = ((st.get("phone") or {}).get("threads") or {}).get("a") or {}
    assert thread.get("msgs"), "纪念日短信在玩家点开的回合落进小手机"
...
def test_anniversary_needs_warmth():
    ...
    st["clock"] = {"day": 30, "slot": 0, "turns_in_slot": 0}
    st["anniv_met"] = {"a": 1}
    out = living.world_tick(STORY, st)   # 没热度 → 不过纪念日
